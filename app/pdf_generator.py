...
class KundliPDFGenerator:
    ...
    def generate(self, output_path: str = None) -> bytes:
        ...
        all_sections = [
            'birth_details', 'kundli_chart', 'navamsa_chart', 'hora_chart',
            'planet_positions', 'houses', 'nakshatras', 'dasha',
            'yogas', 'doshas', 'planet_strengths',
            'career', 'finance', 'health', 'love', 'education', 'family', 'travel',
            'ai_predictions', 'major_charts',
            'gemstones', 'remedies', 'lucky',
        ]
        if self.sections:
        ...
        if 'kundli_chart' in active:
            elements.extend(build_kundli_chart_section(
                self._planets, self._asc['sign']
            ))

        if 'navamsa_chart' in active:
            elements.extend(build_navamsa_chart_section(self._planets))
        ...
        if 'houses' in active:
            elements.extend(build_houses_section(
                self._house_data['houses'], self._pmap
            ))
        ...
        if 'gemstones' in active:
            elements.extend(build_gemstone_section(
                self._planets, self._pmap, self._asc['sign']
            ))
    ...
    def generate_info(self) -> Dict[str, Any]:
        ...
        all_section_meta = {
            ...
            'yogas': 'Yogas',
            'doshas': 'Doshas',
            'planet_strengths': 'Planet Strengths',
